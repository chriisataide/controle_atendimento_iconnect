...
def seed_notifications():
    ...
    scenarios = [
        ...
        (
            ...
            "Ticket #{num} foi atribuído a você: {titulo}",
            "info",
            "person_add",
        ),
        ...
        (
            ...
            "Ticket #{num}: Sistema indisponível - URGENTE",
            "danger",
            "priority_high",
        ),
        (
            ...
            "Ticket #{num} redistribuído pelo sistema de balanceamento",
            "info",
            "person_add",
        ),
        (
            ...
            "Ticket Resolvido",
            "Ticket #{num} foi resolvido pelo agente",
            "success",
            "check_circle",
        ),
        (
            "system_alert",
            "Relatório Gerado",
            "Relatório semanal de performance disponível para download",
            "info",
            "assessment",
        ),
    ]
    ...
    for i, (ntype, title, msg_tpl, color, icon) in enumerate(scenarios):
        ...
        msg = msg_tpl.format(
            num=ticket.numero if ticket else "???", titulo=ticket.titulo if ticket else "Ticket de teste"
        )
        ...
        Notification.objects.create(
            ...
            ticket=ticket,
            created_at=timezone.now() - timedelta(hours=random.randint(0, 48), minutes=random.randint(0, 59)),
        )
        created += 1
