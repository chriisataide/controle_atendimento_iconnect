...
def create_sample_data():
    ...
    if not User.objects.filter(username="agente1").exists():
        agente1 = User.objects.create_user(
            username="agente1", email="agente1@iconnect.com", password="agente123", first_name="João", last_name="Silva"
        )
    ...
    for cat_data in categorias_data:
        categoria, created = CategoriaTicket.objects.get_or_create(
            nome=cat_data["nome"], defaults={"descricao": cat_data["descricao"]}
        )
        if created:
    ...
    for user in agentes:
        perfil, created = PerfilAgente.objects.get_or_create(
            user=user, defaults={"status": "disponivel", "max_tickets_simultaneos": 10}
        )
        if created:
    ...
    for sla_data in sla_policies_data:
        sla, created = SLAPolicy.objects.get_or_create(name=sla_data["name"], defaults=sla_data)
        if created:
    ...
    workflow_rules_data = [
        {
            ...
            "actions": {"assign_to_best_agent": True, "send_notification": ["email", "slack"]},
            "priority": 1,
            "is_active": True,
        },
        {
            ...
            "conditions": {"sla_remaining_hours": 1},
            "actions": {"notify_supervisor": True, "send_notification": ["email", "whatsapp"]},
            "priority": 2,
            "is_active": True,
        },
    ]
    ...
    for rule_data in workflow_rules_data:
        rule, created = WorkflowRule.objects.get_or_create(name=rule_data["name"], defaults=rule_data)
        if created:
    ...
    for kb_item in kb_data:
        kb, created = KnowledgeBase.objects.get_or_create(title=kb_item["title"], defaults=kb_item)
        if created:
    ...
    metrics, created = SystemMetrics.objects.get_or_create(
        ...
        defaults={
            ...
            "agent_productivity": {"tickets_per_agent": 12, "avg_response_time": 2.3},
        },
    )
    if created:
