...
def send_dashboard_update():
    ...
    stats = {
        ...
        "resolved_today": Ticket.objects.filter(status="resolvido", resolvido_em__date=timezone.now().date()).count(),
        "agents_online": PerfilAgente.objects.filter(status="online").count(),
        "timestamp": timezone.now().isoformat(),
    }

    _safe_group_send("dashboard_updates", {"type": "metrics_update", "data": stats})
            ...
            if sla_history:
                ...
                if instance.agente:
                    Notification.objects.create(
                        ...
                        message=(
                            f'Ticket atribuído com prazo SLA: '
                            f'{sla_history.first_response_deadline.strftime("%H:%M")}'
                        ),
                        icon="schedule",
                        color="info",
                        ticket=instance,
...
def track_first_response(sender, instance, **kwargs):
    ...
    if instance.pk:  # Ticket já existe
        try:
            ...
            if (
                old_ticket.status != "em_andamento"
                and instance.status == "em_andamento"
    ...
    if not created and instance.status in ["resolvido", "fechado"]:
        try:
            ...
            if sla_history and not sla_history.resolution_time:
                ...
                first_response_ok = (
                    sla_history.first_response_time
                    and instance.first_response_at <= sla_history.first_response_deadline
                )
                ...
                if sla_history.sla_compliance:
                    ...
                    if instance.agente:
                        Notification.objects.create(
                            ...
                            message="Ticket resolvido fora do prazo SLA estabelecido.",
                            icon="error_outline",
                            color="danger",
                            ticket=instance,
                        )
...
def send_sla_dashboard_update():
    ...
    try:
        ...
        _safe_group_send("sla_dashboard", {"type": "sla_update", "data": dashboard_data})
        ...
        try:
            ...
            movimentacao_original = (
                MovimentacaoEstoque.objects.filter(
                    ticket_relacionado=instance.ticket, produto=instance.produto, tipo_operacao="saida"
                )
                .order_by("-criado_em")
                .first()
            )
