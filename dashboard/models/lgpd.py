...
class LGPDDataRequest(models.Model):
    ...
    request_type = models.CharField(
        max_length=20, choices=REQUEST_TYPES,
    ...
    response = models.TextField(
        blank=True,
        verbose_name="Resposta ao titular"
    )
    # Rastreabilidade
    ip_address = models.GenericIPAddressField(
        null=True, blank=True,
        verbose_name="IP da solicitação"
    )
...
class LGPDAccessLog(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.SET_NULL,
        null=True, related_name='lgpd_access_logs',
    ...
    resource_type = models.CharField(
        ...
        help_text="Ex: Cliente, Fornecedor, PontoDeVenda"
    )
    ...
    fields_accessed = models.JSONField(
        default=list,
        verbose_name="Campos acessados",
        help_text="Lista de campos PII acessados"
    )
    ip_address = models.GenericIPAddressField(
        null=True, blank=True,
