...
class WhatsAppBusinessAccount(models.Model):
    ...
    webhook_verify_token = models.CharField(
        max_length=500, verbose_name="Webhook Verify Token", help_text="Armazenado criptografado"
    )
    ...
    def get_access_token(self):
        ...
        from dashboard.utils.crypto import decrypt_value
...
class WhatsAppContact(models.Model):
    ...
    usuario = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, verbose_name="Usuário Vinculado"
    )
...
class WhatsAppConversation(models.Model):
    ...
    agente = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, verbose_name="Agente Responsável"
    )
    ticket = models.ForeignKey(
        "Ticket", on_delete=models.SET_NULL, null=True, blank=True, verbose_name="Ticket Vinculado"
    )
...
class WhatsAppMessage(models.Model):
    ...
    conversation = models.ForeignKey(
        WhatsAppConversation, on_delete=models.CASCADE, related_name="mensagens", verbose_name="Conversa"
    )
    contact = models.ForeignKey(WhatsAppContact, on_delete=models.CASCADE, verbose_name="Contato")
...
class WhatsAppTemplate(models.Model):
    ...
    status = models.CharField(
        max_length=20, choices=STATUS_TEMPLATE, default="pending", verbose_name="Status", db_index=True
    )
...
class WhatsAppAutoResponse(models.Model):
    ...
    template = models.ForeignKey(
        WhatsAppTemplate, on_delete=models.CASCADE, null=True, blank=True, verbose_name="Template"
    )
...
class WhatsAppAnalytics(models.Model):
    ...
    tempo_resposta_medio = models.DecimalField(
        max_digits=8, decimal_places=2, null=True, blank=True, verbose_name="Tempo Resposta Médio (min)"
    )
...
class WhatsAppWebhookLog(models.Model):
    ...
    account = models.ForeignKey(
        WhatsAppBusinessAccount, on_delete=models.CASCADE, null=True, blank=True, verbose_name="Conta"
    )
