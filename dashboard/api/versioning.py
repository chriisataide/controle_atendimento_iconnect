...
def api_version(version="v1", supported_versions=None):
    ...
    def decorator(view_func):
        ...
        def wrapper(request, *args, **kwargs):
            ...
            requested_version = (
                request.META.get("HTTP_API_VERSION")
                or request.GET.get("version")
                or version  # usar versão padrão se não especificada
            )
            ...
            if not _is_valid_version(requested_version) or requested_version not in valid_versions:
                return JsonResponse(
                    {"error": f"Unsupported API version: {requested_version}", "supported_versions": valid_versions},
                    status=400,
                )
...
def get_api_info():
    ...
    return {"versions": API_VERSIONS, "default_version": DEFAULT_VERSION, "current_time": timezone.now().isoformat()}
...
def validate_api_request(request):
    ...
    version = request.headers.get("API-Version") or request.GET.get("version") or DEFAULT_VERSION
