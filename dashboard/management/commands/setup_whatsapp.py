...
from dashboard.models import WhatsAppAutoResponse, WhatsAppBusinessAccount, WhatsAppTemplate
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--account-name", type=str, default="Conta Principal", help="Nome da conta WhatsApp Business"
        )
        parser.add_argument("--phone-id", type=str, help="Phone Number ID do WhatsApp Business")
        parser.add_argument("--business-id", type=str, help="Business Account ID")

    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS("🔧 Configurando WhatsApp Business..."))
        ...
        if not WhatsAppBusinessAccount.objects.exists():
            ...
            self.stdout.write(self.style.SUCCESS(f"✅ Conta criada: {account.nome}"))
        else:
            account = WhatsAppBusinessAccount.objects.first()
            self.stdout.write(self.style.WARNING(f"ℹ️  Usando conta existente: {account.nome}"))
        ...
        for response_data in auto_responses_data:
            ...
            if created:
                self.stdout.write(self.style.SUCCESS(f"✅ Resposta automática criada: {auto_response.nome}"))

        self.stdout.write("\n" + "=" * 50)
        self.stdout.write(self.style.SUCCESS("🎉 WhatsApp Business configurado com sucesso!"))
