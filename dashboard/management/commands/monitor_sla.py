...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if dry_run:
            self.stdout.write(
                self.style.WARNING('MODO SIMULAÇÃO ATIVADO - Nenhuma alteração será feita')
            )
    ...
    def _display_results(self, stats, verbose):
        ...
        self.stdout.write(
            self.style.SUCCESS(f'✓ Monitoramento SLA concluído com sucesso!')
        )
        ...
        if stats['warnings_sent'] > 0:
            self.stdout.write(
                self.style.WARNING(f'  ⚠️  Alertas enviados: {stats["warnings_sent"]}')
            )
        
        if stats['breaches_detected'] > 0:
            self.stdout.write(
                self.style.ERROR(f'  🚨 Violações detectadas: {stats["breaches_detected"]}')
            )
        
        if stats['escalations_made'] > 0:
            self.stdout.write(
                self.style.HTTP_INFO(f'  📈 Escalações realizadas: {stats["escalations_made"]}')
            )
        
        if stats['errors'] > 0:
            self.stdout.write(
                self.style.ERROR(f'  ❌ Erros encontrados: {stats["errors"]}')
            )
