...
from dashboard.models import CategoriaTicket, Cliente, Notification, Ticket
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for dados in clientes_dados:
            cliente, created = Cliente.objects.get_or_create(email=dados["email"], defaults=dados)
            if created:
        ...
        for dados in categorias_dados:
            categoria, created = CategoriaTicket.objects.get_or_create(nome=dados["nome"], defaults=dados)
            if created:
