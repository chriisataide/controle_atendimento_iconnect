...
from dashboard.models import (
    SLAPolicy, CategoriaTicket, PrioridadeTicket, 
    Ticket, StatusTicket
)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS('✅ Sistema SLA configurado com sucesso!')
        )
    ...
    def create_sla_policies(self):
        ...
        if not supervisor:
            self.stdout.write(
                self.style.WARNING('⚠️  Nenhum usuário supervisor encontrado para escalação')
            )
        ...
        for ticket in tickets_sem_sla:
            try:
                ...
                if sla_history:
                    count += 1
                    self.stdout.write(
                        f'🎯 SLA aplicado ao ticket #{ticket.numero}'
                    )
            except Exception as e:
                self.stdout.write(
                    self.style.ERROR(f'❌ Erro ao aplicar SLA ao ticket #{ticket.numero}: {str(e)}')
                )
        
        if count > 0:
            self.stdout.write(
                self.style.SUCCESS(f'✅ SLA aplicado a {count} tickets existentes')
            )
        else:
