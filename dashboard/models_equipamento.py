...
class Equipamento(models.Model):
    ...
    numero_serie = models.CharField(
        "Número de Série", max_length=100, unique=True, db_index=True,
        help_text="Identificador único do equipamento"
    )
    modelo = models.CharField("Modelo", max_length=100)
    marca = models.CharField("Marca", max_length=100, blank=True)
    tipo = models.CharField(
        "Tipo de Equipamento", max_length=100, db_index=True,
        help_text="Ex: Roteador, Switch, ONT, Cabo, OLT, etc."
    )
    descricao = models.TextField("Descrição", blank=True)
    patrimonio = models.CharField(
        "Nº Patrimônio", max_length=50, blank=True,
        help_text="Código interno de patrimônio"
    )
    ...
    local_instalacao = models.CharField(
        ...
        help_text="Detalhamento: sala, rack, posição"
    )

    # Status e datas
    status = models.CharField(
        max_length=20, choices=StatusEquipamento.choices,
        default=StatusEquipamento.EM_ESTOQUE, db_index=True
    )
    ...
    def atualizar_contadores(self):
        ...
        self.total_trocas = self.historico.filter(
...
class HistoricoEquipamento(models.Model):
    ...
    equipamento = models.ForeignKey(
        Equipamento, on_delete=models.CASCADE, related_name='historico'
    )
    ...
    realizado_por = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True,
        related_name='movimentacoes_equipamento_realizadas'
    )
...
class AlertaEquipamento(models.Model):
    ...
    equipamento = models.ForeignKey(
        Equipamento, on_delete=models.CASCADE, related_name='alertas'
    )
    tipo = models.CharField(max_length=30, choices=TipoAlerta.choices, db_index=True)
    severidade = models.CharField(
        max_length=10, choices=Severidade.choices, default=Severidade.WARNING
    )
    ...
    valor_atual = models.IntegerField(
        "Valor Atual", default=0,
        help_text="Ex: quantidade de chamados que disparou o alerta"
    )
    limiar = models.IntegerField(
        "Limiar", default=3,
        help_text="Limite configurado para este tipo de alerta"
    )
    ...
    resolvido_por = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True,
        related_name='alertas_equipamento_resolvidos'
    )
...
class ConfiguracaoAlertaEquipamento(models.Model):
    ...
    chamados_limiar = models.PositiveIntegerField(
        "Chamados para Alerta", default=3,
    ...
    garantia_dias_aviso = models.PositiveIntegerField(
        "Aviso de Garantia (dias)", default=30,
        validators=[MinValueValidator(1)],
