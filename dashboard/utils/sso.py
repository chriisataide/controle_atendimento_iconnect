...
class SSOProvider(models.Model):
    ...
    client_secret = models.CharField(
        max_length=500, blank=True, verbose_name="Client Secret", help_text="Armazenado criptografado"
    )
    ...
    scopes = models.CharField(max_length=200, default="openid email profile", help_text="Escopos separados por espaço")
    ...
    attr_email = models.CharField(max_length=100, default="email", help_text="Nome do atributo para email")
    attr_first_name = models.CharField(
        max_length=100, default="given_name", help_text="Nome do atributo para primeiro nome"
    )
    attr_last_name = models.CharField(
        max_length=100, default="family_name", help_text="Nome do atributo para sobrenome"
    )
    attr_username = models.CharField(
        max_length=100, default="preferred_username", help_text="Nome do atributo para username"
    )
    ...
    domain_whitelist = models.TextField(
        blank=True, help_text="Domínios permitidos (um por linha). Vazio = qualquer domínio"
    )
    ...
    def _provision_user(self, provider, email, username, first_name, last_name):
        ...
        try:
            ...
            try:
                ...
                UserRole.objects.get_or_create(user=user, defaults={"role": provider.default_role})
