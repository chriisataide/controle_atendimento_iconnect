...
class WhatsAppBusinessAccountAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Informações Básicas", {"fields": ("nome", "phone_number_id", "business_account_id", "ativo")}),
        (
            "Configurações de API",
            {"fields": ("access_token", "webhook_verify_token", "webhook_url"), "classes": ["collapse"]},
        ),
...
class WhatsAppContactAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Informações do Contato", {"fields": ("whatsapp_id", "phone_number", "nome", "profile_name")}),
        ("Vinculação", {"fields": ("usuario", "tags", "bloqueado")}),
        ("Metadados", {"fields": ("metadados",), "classes": ["collapse"]}),
        ("Timestamps", {"fields": ("primeiro_contato", "ultimo_contato"), "classes": ["collapse"]}),
    )
...
class WhatsAppConversationAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Informações da Conversa", {"fields": ("uuid", "account", "contact", "titulo", "estado")}),
        ("Atribuição", {"fields": ("agente", "ticket")}),
        ("Contexto", {"fields": ("contexto",), "classes": ["collapse"]}),
        ("Timestamps", {"fields": ("iniciada_em", "encerrada_em"), "classes": ["collapse"]}),
    )
...
class WhatsAppMessageAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Informações da Mensagem", {"fields": ("whatsapp_message_id", "conversation", "contact", "agente")}),
        ("Conteúdo", {"fields": ("direcao", "tipo", "conteudo", "media_url", "media_mime_type")}),
        ("Status", {"fields": ("status", "processada", "erro")}),
        ("Metadados", {"fields": ("metadata", "timestamp"), "classes": ["collapse"]}),
    )
...
class WhatsAppTemplateAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Informações Básicas", {"fields": ("account", "nome", "categoria", "idioma", "status", "ativo")}),
        ("Conteúdo", {"fields": ("descricao", "conteudo")}),
        ("Timestamps", {"fields": ("criado_em", "atualizado_em"), "classes": ["collapse"]}),
    )
...
class WhatsAppAutoResponseAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Informações Básicas", {"fields": ("account", "nome", "ativo", "prioridade")}),
        ("Configuração do Gatilho", {"fields": ("tipo_trigger", "trigger_value")}),
        ("Resposta", {"fields": ("template", "mensagem_texto")}),
        ("Timestamp", {"fields": ("criado_em",), "classes": ["collapse"]}),
    )
...
class WhatsAppAnalyticsAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Informações", {"fields": ("account", "data")}),
        (
...
class WhatsAppWebhookLogAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Informações do Webhook", {"fields": ("account", "tipo_evento", "processado")}),
        ("Dados da Requisição", {"fields": ("payload", "ip_origem", "user_agent"), "classes": ["collapse"]}),
        ("Erro", {"fields": ("erro",), "classes": ["collapse"]}),
