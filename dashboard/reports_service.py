...
class AdvancedReportsService:
    ...
    async def _generate_agent_performance(self, config: ReportConfig) -> Dict[str, Any]:
        ...
        def get_agent_stats():
            ...
            tickets = Ticket.objects.filter(
                created_at__range=[config.start_date, config.end_date]
            )
        ...
        fastest_resolvers = sorted(
            [s for s in agent_stats if s['avg_resolution_time_hours'] > 0], 
            key=lambda x: x['avg_resolution_time_hours']
        )[:5]
    ...
    async def _generate_tickets_summary(self, config: ReportConfig) -> Dict[str, Any]:
        ...
        return {
            'summary': {
                ...
                'resolution_rate': round(resolution_rate, 2),
                'escalation_rate': round((data['escalated_tickets'] / data['total_tickets'] * 100), 2) if data['total_tickets'] > 0 else 0,
            ...
            'trends': {
                'daily_creation': data['daily_tickets']
            },
    ...
    async def _generate_sla_analysis(self, config: ReportConfig) -> Dict[str, Any]:
        ...
        def get_sla_data():
            ...
            for priority in priorities:
                ...
                if priority_total > 0:
                    priority_met = priority_tickets.filter(
                        status='FECHADO',
                        resolved_at__lte=F('sla_deadline')
                    ).count()
            ...
            for agent in agents:
                ...
                if agent_total > 0:
                    agent_met = agent_tickets.filter(
                        status='FECHADO',
                        resolved_at__lte=F('sla_deadline')
                    ).count()
            ...
            sla_trend = tickets.filter(
                status='FECHADO'
            ).annotate(
                week=TruncWeek('resolved_at')
        ...
        return {
            ...
            'breakdown': {
                'by_priority': data['sla_by_priority'],
                'by_agent': data['sla_by_agent']
            },
            'trends': {
                'weekly_compliance': data['sla_trend']
            },
            'charts': {
                'compliance_overview': {
    ...
    def _calculate_sla_compliance(self, tickets_queryset) -> float:
        ...
        met_sla = tickets_queryset.filter(
            status='FECHADO',
            resolved_at__lte=F('sla_deadline')
        ).count()
