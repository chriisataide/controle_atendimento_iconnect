...
from ..models import WhatsAppAutoResponse, WhatsAppBusinessAccount, WhatsAppContact, WhatsAppTemplate
...
class WhatsAppAccountForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            "nome": forms.TextInput(attrs={"class": "form-control", "placeholder": "Nome da conta"}),
            "phone_number_id": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "Phone Number ID do WhatsApp"}
            ),
            "business_account_id": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "Business Account ID"}
            ),
            "access_token": forms.Textarea(
                attrs={"class": "form-control", "rows": 3, "placeholder": "Access Token da API"}
            ),
            "webhook_verify_token": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "Token de verificação do webhook"}
            ),
            "webhook_url": forms.URLInput(attrs={"class": "form-control", "placeholder": "URL do webhook"}),
...
class WhatsAppTemplateForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = ["account", "nome", "categoria", "idioma", "conteudo", "descricao", "ativo"]
        widgets = {
            "account": forms.Select(attrs={"class": "form-select"}),
            "nome": forms.TextInput(attrs={"class": "form-control", "placeholder": "Nome do template"}),
            "categoria": forms.Select(attrs={"class": "form-select"}),
            "idioma": forms.TextInput(attrs={"class": "form-control", "value": "pt_BR"}),
            "conteudo": forms.Textarea(
                attrs={"class": "form-control", "rows": 5, "placeholder": "Conteúdo do template em JSON"}
            ),
            "descricao": forms.Textarea(
                attrs={"class": "form-control", "rows": 3, "placeholder": "Descrição do template"}
            ),
            "ativo": forms.CheckboxInput(attrs={"class": "form-check-input"}),
        }
...
class WhatsAppAutoResponseForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            "account": forms.Select(attrs={"class": "form-select"}),
            "nome": forms.TextInput(attrs={"class": "form-control", "placeholder": "Nome da resposta automática"}),
            "tipo_trigger": forms.Select(attrs={"class": "form-select"}),
            "trigger_value": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "Valor do gatilho (ex: palavra-chave)"}
            ),
            "template": forms.Select(attrs={"class": "form-select"}),
            "mensagem_texto": forms.Textarea(
                attrs={"class": "form-control", "rows": 4, "placeholder": "Mensagem de texto (se não usar template)"}
            ),
            "ativo": forms.CheckboxInput(attrs={"class": "form-check-input"}),
            "prioridade": forms.NumberInput(attrs={"class": "form-control", "min": 0, "max": 100}),
        }
...
class WhatsAppMessageForm(forms.Form):
    ...
    tipo = forms.ChoiceField(choices=TIPOS_MENSAGEM, widget=forms.Select(attrs={"class": "form-select"}))

    mensagem = forms.CharField(
        widget=forms.Textarea(attrs={"class": "form-control", "rows": 4, "placeholder": "Digite sua mensagem..."}),
        required=False,
    )
...
class WhatsAppBulkMessageForm(forms.Form):
    ...
    contacts = forms.ModelMultipleChoiceField(
        queryset=WhatsAppContact.objects.filter(bloqueado=False), widget=forms.CheckboxSelectMultiple, label="Contatos"
    )
...
class WhatsAppAnalyticsFilterForm(forms.Form):
    ...
    period = forms.ChoiceField(choices=PERIOD_CHOICES, widget=forms.Select(attrs={"class": "form-select"}), initial=30)

    data_inicio = forms.DateField(
        widget=forms.DateInput(attrs={"class": "form-control", "type": "date"}), required=False
    )

    data_fim = forms.DateField(widget=forms.DateInput(attrs={"class": "form-control", "type": "date"}), required=False)
