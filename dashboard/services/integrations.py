...
from ..models import Cliente, InteracaoTicket, Ticket
...
def _verify_whatsapp_signature(request):
    ...
    computed_signature = hmac.new(app_secret.encode("utf-8"), request.body, hashlib.sha256).hexdigest()
    ...
    try:
        ...
        for message in messages:
            ...
            ticket = Ticket.objects.filter(
                cliente=cliente, status__in=["aberto", "em_andamento", "aguardando_cliente"]
            ).first()
            ...
            if not ticket:
                ...
                if bot_response:
                    InteracaoTicket.objects.create(
                        ticket=ticket, usuario=None, tipo="sistema", conteudo=bot_response, publico=True
                    )
                ...
                InteracaoTicket.objects.create(
                    ticket=ticket, usuario=None, tipo="cliente", conteudo=message_body, publico=True
                )
...
def _process_slack_command(command_data):
    ...
    try:
        ...
        return JsonResponse(
            {"response_type": "ephemeral", "text": "Comando não reconhecido. Use /ticket [numero] ou /status"}
        )

    except Exception as e:
        logger.error(f"Erro ao processar comando Slack: {str(e)}")
        return JsonResponse({"response_type": "ephemeral", "text": "Erro interno do servidor."})
...
def _handle_slack_ticket_command(text):
    ...
    try:
        if not text:
            ...
            tickets_recentes = Ticket.objects.filter(status__in=["aberto", "em_andamento"]).order_by("-criado_em")[:5]

            if not tickets_recentes:
                return JsonResponse({"response_type": "ephemeral", "text": "Nenhum ticket ativo encontrado."})
            ...
            return JsonResponse({"response_type": "in_channel", "text": "Tickets Ativos:", "attachments": attachments})
            ...
            try:
                        ...
                        "attachments": [
                            {
                                ...
                                "fields": [
                                    ...
                                    {
                                        "title": "Agente",
                                        "value": ticket.agente.username if ticket.agente else "Não atribuído",
                                        "short": True,
                                    },
                ...
                return JsonResponse({"response_type": "ephemeral", "text": f"Ticket {text.upper()} não encontrado."})
...
def _handle_slack_status_command():
    ...
    try:
        ...
        return JsonResponse({"response_type": "ephemeral", "text": "Erro ao obter status do sistema."})
