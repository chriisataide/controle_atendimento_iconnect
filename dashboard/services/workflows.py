...
from ..models import InteracaoTicket, WorkflowExecution, WorkflowRule
...
class WorkflowEngine:
    ...
    def _get_applicable_rules(self, ticket, trigger_event):
        ...
        try:
            ...
            rules = WorkflowRule.objects.filter(is_active=True, trigger_event=trigger_event)
    ...
    def _execute_rule(self, rule, ticket, user):
        ...
        try:
            ...
            for action_type, action_config in actions.items():
                ...
                results["actions_executed"].append({"type": action_type, "result": result})
    ...
    def _execute_action(self, action_type, action_config, ticket, user):
        ...
        try:
            if action_type == "change_status":
                ...
                # Aumentar prioridade
                priority_escalation = {"baixa": "media", "media": "alta", "alta": "critica"}
    ...
    def get_available_workflows(self, categoria=None):
        ...
        try:
            ...
            if categoria and categoria.nome.lower() in default_workflows:
                ...
                return default_workflows.get(
                    "suporte_tecnico", ["aberto", "em_andamento", "aguardando_cliente", "resolvido", "fechado"]
                )
    ...
    def get_workflow_metrics(self, days=30):
        ...
        try:
            ...
            executions = WorkflowExecution.objects.filter(created_at__gte=cutoff_date)

            total_executions = executions.count()
            successful_executions = executions.filter(execution_result__icontains="actions_executed").count()

            # Top regras mais executadas
            top_rules = executions.values("rule__name").annotate(count=models.Count("id")).order_by("-count")[:5]
            ...
            return {"total_executions": 0, "successful_executions": 0, "success_rate": 0, "top_rules": []}
    ...
    def get_rule_template(self, template_name):
        ...
        templates = {
            ...
            "escalate_old_tickets": {
                ...
                "conditions": {"status": ["aberto", "em_andamento"], "time_since_creation": {"min_hours": 24}},
                "actions": {
                    "escalate": {"level": 1},
                    "add_comment": {
            ...
            "close_resolved_tickets": {
                ...
                "trigger_event": "ticket_updated",
                "conditions": {"status": ["resolvido"], "time_since_creation": {"min_hours": 48}},
                "actions": {
