...
from ..models import Notification, SLAAlert, SLAHistory, StatusTicket, Ticket
...
class SLAMonitor:
    ...
    def monitor_ticket(self, ticket: Ticket) -> Dict[str, bool]:
        ...
        """
        result = {"warning_sent": False, "breach_detected": False, "escalation_made": False}
        ...
        if current_status == "warning" and not sla_history.warning_sent:
            ...
            if (
                sla_history.sla_policy.escalation_enabled
                and not sla_history.escalated
                and self.should_escalate(sla_history)
            ):
    ...
    def should_escalate(self, sla_history: SLAHistory) -> bool:
        ...
        if sla_history.ticket.prioridade == "critica" and sla_history.status == "breached":
            return True
        ...
        try:
            ...
            if ticket.agente:
                Notification.objects.create(
                    ...
                    ticket=ticket,
                    metadata={"alert_type": alert_type, "sla_alert_id": alert.id},
                )
            ...
            if alert_type == "breach" and ticket.sla_policy and ticket.sla_policy.escalation_to:
                Notification.objects.create(
                    ...
                    color="danger",
                    ticket=ticket,
                    metadata={"alert_type": alert_type, "sla_alert_id": alert.id, "requires_action": True},
                )
    ...
    def _send_escalation_notifications(self, ticket: Ticket, alert: SLAAlert, old_agent: User, new_agent: User):
        ...
        try:
            ...
            Notification.objects.create(
                ...
                color="danger",
                ticket=ticket,
                metadata={"escalated": True, "previous_agent": old_agent.username if old_agent else None},
            )
