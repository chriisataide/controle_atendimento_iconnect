...
class TicketService:
    ...
    def get_tickets_with_filters(self, filters: Dict, user=None) -> List[Ticket]:
        ...
        queryset = Ticket.objects.select_related(
            'cliente', 'agente', 'categoria', 'sla_policy'
        ).prefetch_related('interacoes', 'anexos')
    ...
    def _auto_assign_agent(self, categoria: str) -> Optional[PerfilAgente]:
        ...
        try:
            ...
            agentes = PerfilAgente.objects.filter(
                status='online',
                especialidades__nome__icontains=categoria
            ).annotate(
                ticket_count=Count('user__assigned_tickets', 
