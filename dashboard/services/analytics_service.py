...
from ..models import Ticket, Cliente, PerfilAgente, InteracaoTicket
...
class AnalyticsService:
    ...
    def get_agent_performance(self, agent_id: Optional[int] = None) -> Dict:
        ...
        return {
            ...
            'team_average': self._calculate_team_averages(agent_stats)
        }
    ...
    def get_category_analysis(self) -> Dict:
        ...
        """
        category_stats = Ticket.objects.values('categoria__nome').annotate(
            total=Count('id'),
            resolved=Count('id', filter=Q(status='resolvido')),
            avg_resolution_time=Avg(
                F('resolvido_em') - F('criado_em'),
                filter=Q(resolvido_em__isnull=False)
            ),
    ...
    def _calculate_first_response_time(self, tickets) -> Optional[float]:
        ...
        for ticket in tickets:
            first_response = InteracaoTicket.objects.filter(
                ticket=ticket,
                tipo='resposta'
    ...
    def _get_common_issues(self, tickets) -> List[Dict]:
        ...
        return list(tickets.values('categoria__nome').annotate(
            count=Count('id')
        ).order_by('-count')[:5])
