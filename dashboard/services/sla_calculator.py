...
from ..models import SLAPolicy, Ticket, SLAHistory
...
class SLACalculator:
    ...
    def calculate_deadline(self, 
        ...
        while remaining_minutes > 0:
            ...
            end_of_day = current_time.replace(
                hour=policy.end_hour.hour,
                minute=policy.end_hour.minute,
                second=0,
                microsecond=0
            )
    ...
    def calculate_sla_deadlines(self, ticket: Ticket) -> Optional[Tuple[datetime, datetime, datetime]]:
        ...
        first_response_deadline = self.calculate_deadline(
            start_time, policy.first_response_time, policy
        )
        
        # Calcula prazo de resolução
        resolution_deadline = self.calculate_deadline(
            start_time, policy.resolution_time, policy
        )
        
        # Calcula prazo de escalação
        escalation_deadline = self.calculate_deadline(
            start_time, policy.escalation_time, policy
        )
    ...
    def calculate_sla_metrics(self, sla_history: SLAHistory) -> dict:
        ...
        if metrics['first_response_sla_met'] is not None and metrics['resolution_sla_met'] is not None:
            metrics['overall_sla_compliance'] = (
                metrics['first_response_sla_met'] and metrics['resolution_sla_met']
            )
        
        # Tempo até violação
        if ticket.status not in ['resolvido', 'fechado']:
            if not ticket.first_response_at:
                metrics['time_to_breach'] = max(
                    timedelta(0),
                    sla_history.first_response_deadline - now
                )
            else:
                metrics['time_to_breach'] = max(
                    timedelta(0),
                    sla_history.resolution_deadline - now
                )
