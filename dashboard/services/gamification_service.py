...
class GamificationService:
    ...
    def award_points(self, user: User, action: str, extra_points: int = 0):
        ...
        lb, created = AgentLeaderboard.objects.get_or_create(usuario=user, defaults={"periodo": periodo})
    ...
    def check_badges(self, user: User):
        ...
        from dashboard.models import AgentBadge, GamificationBadge, Ticket
        ...
        for badge in badges:
            ...
            if metric == "tickets_resolved":
                count = Ticket.objects.filter(agente=user, status__in=["resolvido", "fechado"]).count()
                ...
                try:
                    ...
                    avg = AvaliacaoSatisfacao.objects.filter(ticket__agente=user).aggregate(
                ...
                first_ticket = Ticket.objects.filter(agente=user).order_by("criado_em").first()
                if first_ticket:
                ...
                count = (
                    ...
                    .annotate(
                        resolution_time=ExpressionWrapper(
                            F("resolvido_em") - F("criado_em"), output_field=DurationField()
                        )
                    )
                    .filter(resolution_time__lt=timedelta(hours=1))
                    .count()
                )
                earned = count >= threshold
    ...
    def update_leaderboard(self):
        ...
        for agent in agents:
            ...
            lb, created = AgentLeaderboard.objects.get_or_create(usuario=agent, defaults={"periodo": periodo})
            ...
            try:
                ...
                avg = AvaliacaoSatisfacao.objects.filter(ticket__agente=agent, criado_em__gte=last_30d).aggregate(
                    avg=Avg("nota_atendimento")
                )["avg"]
            ...
            if resolved_with_time.exists():
                avg_delta = resolved_with_time.aggregate(avg=Avg(F("resolvido_em") - F("criado_em")))["avg"]
