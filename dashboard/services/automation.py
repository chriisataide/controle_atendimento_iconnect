...
from ..models import InteracaoTicket, PerfilAgente, Ticket
...
class AutoAssignmentEngine:
    ...
    def _get_available_agents(self, categoria):
        ...
        try:
            perfis_agentes = PerfilAgente.objects.filter(
                status__in=["online", "ocupado"], user__is_active=True
            ).select_related("user")
            ...
            for perfil in perfis_agentes:
                ...
                tickets_ativos = Ticket.objects.filter(
                    agente=perfil.user, status__in=["aberto", "em_andamento", "aguardando_cliente"]
                ).count()
    ...
    def _calculate_best_agent(self, agentes, ticket):
        ...
        for perfil_agente in agentes:
            ...
            tickets_ativos = Ticket.objects.filter(
                agente=perfil_agente.user, status__in=["aberto", "em_andamento"]
            ).count()
            ...
            tickets_categoria = Ticket.objects.filter(
                agente=perfil_agente.user, categoria=ticket.categoria, status="fechado"
            ).count()
            ...
            tickets_resolvidos = Ticket.objects.filter(agente=perfil_agente.user, status="fechado").count()
