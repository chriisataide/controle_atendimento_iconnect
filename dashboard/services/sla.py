...
from ..models import SLAPolicy, SLAViolation, Ticket
...
class SLAManager:
    ...
    def calculate_sla_deadline(self, ticket):
        ...
        try:
            ...
            sla_policy = SLAPolicy.objects.filter(categoria=ticket.categoria, prioridade=ticket.prioridade).first()
            ...
            if sla_policy:
                ...
                response_time_hours = response_times.get(ticket.prioridade, {"hours": 24})["hours"]

            # Calcular deadline considerando apenas horário comercial
            deadline = self._calculate_business_deadline(ticket.criado_em, response_time_hours)
    ...
    def check_sla_status(self, ticket):
        ...
        try:
            ...
            return {"status": "unknown", "deadline": None, "time_remaining": None, "time_remaining_formatted": "N/A"}
    ...
    def get_sla_violations(self, days=7):
        ...
        try:
            ...
            violations = SLAViolation.objects.filter(created_at__gte=cutoff_date).select_related(
    ...
    def get_sla_metrics(self, days=30):
        ...
        try:
            ...
            # Tickets do período
            tickets = Ticket.objects.filter(criado_em__gte=cutoff_date, status__in=["resolvido", "fechado"])
            ...
            violations = SLAViolation.objects.filter(created_at__gte=cutoff_date).count()
...
def monitor_sla_violations():
    ...
    try:
        ...
        active_tickets = Ticket.objects.filter(status__in=["aberto", "em_andamento", "aguardando_cliente"]).exclude(
        ...
        for ticket in active_tickets:
            ...
            if sla_status["status"] == "violated":
                ...
                existing_violation = SLAViolation.objects.filter(
                    ticket=ticket, violation_type="deadline_missed"
                ).exists()
