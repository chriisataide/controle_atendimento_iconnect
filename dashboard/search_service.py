...
class AdvancedSearchService:
    ...
    def _apply_filters(self, queryset, filters: Dict[str, Any]):
        ...
        if 'assigned_to' in filters:
            assigned_list = filters['assigned_to'] if isinstance(filters['assigned_to'], list) else [filters['assigned_to']]
    ...
    def _apply_text_search(self, queryset, query: str):
        ...
        if search_type == 'ticket_number':
            ...
            return queryset.filter(
                Q(customer__email__iexact=query) |
            ...
            return queryset.filter(
                customer__phone__icontains=clean_phone
            )
    ...
    def _get_search_stats(self, base_queryset, filters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        status_counts = base_queryset.values('status').annotate(
    ...
    def get_search_suggestions(self, partial_query: str, limit: int = 10) -> List[str]:
        ...
        if len(partial_query) >= 2:
            ...
            title_suggestions = Ticket.objects.filter(
            ...
            customer_suggestions = Customer.objects.filter(
                name__icontains=partial_query
            ).values_list('name', flat=True).distinct()[:limit//3]
            
            suggestions.extend(customer_suggestions)
            
            # Sugestões de números de ticket
            ticket_suggestions = Ticket.objects.filter(
                ticket_number__icontains=partial_query
            ).values_list('ticket_number', flat=True).distinct()[:limit//3]
