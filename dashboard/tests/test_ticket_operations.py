...
class TicketMergeTest(TestCase):
    ...
    def setUp(self):
        ...
        self.target = Ticket.objects.create(
            cliente=self.cliente, agente=self.user, categoria=self.cat,
            titulo='Ticket Alvo', descricao='Desc target',
        )
        self.source1 = Ticket.objects.create(
            cliente=self.cliente, agente=self.user, categoria=self.cat,
            titulo='Ticket Source 1', descricao='Desc source 1',
        )
        self.source2 = Ticket.objects.create(
            cliente=self.cliente, agente=self.user, categoria=self.cat,
            titulo='Ticket Source 2', descricao='Desc source 2',
        )
        # Add interaction on source
        InteracaoTicket.objects.create(
            ticket=self.source1, usuario=self.user,
            mensagem='Interação no source 1', tipo='resposta',
        )

    def test_merge_success(self):
        result = self.ops.merge_tickets(
            self.target.id, [self.source1.id, self.source2.id], self.user, 'Duplicatas'
        )
    ...
    def test_merge_already_closed_excluded(self):
        ...
        self.source1.save()
        result = self.ops.merge_tickets(
            self.target.id, [self.source1.id, self.source2.id], self.user
        )
        self.assertTrue(result['success'])
...
class TicketSplitTest(TestCase):
    ...
    def setUp(self):
        ...
        self.original = Ticket.objects.create(
            cliente=self.cliente, agente=self.user, categoria=self.cat,
            titulo='Ticket Original', descricao='Descricao grande com vários problemas',
            prioridade='alta',
        )
...
class TicketParentChildTest(TestCase):
    ...
    def setUp(self):
        ...
        self.parent = Ticket.objects.create(
            cliente=self.cliente, agente=self.user, categoria=self.cat,
            titulo='Ticket Pai', descricao='Instalação completa',
        )
    ...
    def test_remove_sub_ticket(self):
        ...
        res = self.ops.add_sub_ticket(
            self.parent.id, {'titulo': 'Temporario'}, self.user,
        )
...
class TicketOperationsViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.ticket1 = Ticket.objects.create(
            cliente=self.cliente_obj, agente=self.user, categoria=self.cat,
