...
def _calcular_tempo_medio_resposta(tickets_qs):
    ...
    tickets_com_interacao = (
        tickets_qs.filter(interacoes__isnull=False)
        .annotate(primeira_resposta=Min("interacoes__criado_em"))
...
class ClientePortalView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if is_admin:
            ...
            context["top_clientes"] = Cliente.objects.annotate(num_tickets=Count("tickets")).order_by("-num_tickets")[
                :10
            ]
        else:
            try:
...
class ClienteListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["segmentos"] = (
            all_clients.exclude(Q(segmento="") | Q(segmento__isnull=True))
