...
def analytics_dashboard(request):
    ...
    # Taxa de Resolução
    resolvidos_mes = Ticket.objects.filter(criado_em__date__gte=inicio_mes, status="resolvido").count()
    taxa_resolucao = (resolvidos_mes / tickets_mes * 100) if tickets_mes > 0 else 0

    # Tempo Médio de Resolução
    tempo_medio = Ticket.objects.filter(status="resolvido", criado_em__date__gte=inicio_mes).aggregate(
    ...
    sla_total = Ticket.objects.filter(criado_em__date__gte=inicio_mes, sla_deadline__isnull=False).count()
    sla_performance = (sla_ok / sla_total * 100) if sla_total > 0 else 0

    # Distribuição por Categoria
    categorias = (
        Ticket.objects.filter(criado_em__date__gte=inicio_mes)
        .values("categoria")
        .annotate(
            total=Count("id"),
    ...
    agentes = (
        Ticket.objects.filter(criado_em__date__gte=inicio_mes, agente__isnull=False)
        .values("agente__first_name", "agente__last_name")
        .annotate(
    ...
    top_clientes = Cliente.objects.annotate(
        total_tickets=Count("tickets"), tickets_mes=Count("tickets", filter=Q(tickets__criado_em__date__gte=inicio_mes))
    ...
    satisfacao_categoria = (
        Ticket.objects.filter(criado_em__date__gte=inicio_mes, avaliacaosatisfacao__isnull=False)
        .values("categoria")
        .annotate(
            satisfacao_media=Avg(
                Cast(
                    F("avaliacaosatisfacao__nota_atendimento")
                    + F("avaliacaosatisfacao__nota_resolucao")
                    + F("avaliacaosatisfacao__nota_tempo"),
                    FloatField(),
                )
                / Value(3.0)
            ),
...
def relatorio_detalhado(request):
    ...
    # Tempo médio de resolução
    tempo_medio = tickets.filter(status="resolvido").aggregate(tempo=Avg(F("resolvido_em") - F("criado_em")))["tempo"]

    # Satisfação média
    satisfacao_media = tickets.filter(avaliacaosatisfacao__isnull=False).aggregate(
