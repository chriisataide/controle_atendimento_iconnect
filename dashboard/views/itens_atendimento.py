...
def api_add_item_atendimento(request):
    ...
    try:
        ...
        if ticket.status not in ["aberto", "em_andamento"]:
            return JsonResponse(
                {"success": False, "message": "Só é possível adicionar itens em tickets abertos ou em andamento"}
            )
        ...
        item_existente = ItemAtendimento.objects.filter(ticket=ticket, produto=produto).first()
...
def api_listar_itens_atendimento(request, ticket_id):
    ...
    try:
        ticket = get_object_or_404(Ticket, id=ticket_id)
        itens = (
            ItemAtendimento.objects.filter(ticket=ticket)
        ...
        return JsonResponse({"success": True, "itens": itens_data, "resumo": resumo})
...
def api_remover_item_atendimento(request, item_id):
    ...
    try:
        ...
        if item.ticket.status not in ["em_andamento"]:
            return JsonResponse(
                {"success": False, "message": "Só é possível remover itens de tickets que estão em andamento"}
            )
...
def relatorio_itens_atendimento(request):
    ...
    queryset = ItemAtendimento.objects.select_related("produto", "produto__categoria", "ticket", "ticket__agente")
    ...
    produtos_populares = (
        queryset.values("produto__nome", "produto__codigo", "tipo_item")
        .annotate(
    ...
    resumo_categorias = (
        queryset.values("produto__categoria__nome")
        .annotate(total_quantidade=Sum("quantidade"), total_valor=Sum(valor_total_expr), total_itens=Count("id"))
        .order_by("-total_valor")
    )

    resumo_agentes = (
        queryset.values("ticket__agente__first_name", "ticket__agente__last_name")
        .annotate(
...
def api_estatisticas_financeiras_ticket(request, ticket_id):
    ...
    try:
        ...
        detalhes_tipos = itens.values("tipo_item").annotate(quantidade=Count("id"), valor_total=Sum("valor_total"))
