...
from ..api.versioning import APIResponseTransformer, api_version
...
class PontoDeVendaListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx["top_estados"] = all_pdvs.values("estado").annotate(total=Count("id")).order_by("-total")[:5]

        # UFs disponíveis para filtro
        ctx["ufs_disponiveis"] = all_pdvs.values_list("estado", flat=True).distinct().order_by("estado")

        # Clientes disponíveis para filtro
        ctx["clientes_disponiveis"] = Cliente.objects.filter(pontos_de_venda__isnull=False).distinct().order_by("nome")
...
class ProfileView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        perfil, created = PerfilUsuario.objects.get_or_create(user=user, defaults={"telefone": ""})
        ...
        recent = InteracaoTicket.objects.filter(usuario=user).select_related("ticket").order_by("-criado_em")[:5]

        activities = []
        color_map = {
            "resposta": "info",
            "nota_interna": "warning",
            "sistema": "secondary",
    ...
    def _handle_profile_update(self, request):
        ...
        try:
            perfil, created = PerfilUsuario.objects.get_or_create(user=request.user, defaults={"telefone": ""})
...
def update_agent_status(request):
    ...
    if request.method == "POST":
        ...
        if new_status not in valid_statuses:
            return JsonResponse({"success": False, "message": f'Status inválido. Use: {", ".join(valid_statuses)}'})

        try:
            perfil_agente, created = PerfilAgente.objects.get_or_create(
                user=request.user, defaults={"status": new_status}
            )
            if not created:
