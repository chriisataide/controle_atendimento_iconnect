...
def chatbot_dashboard(request):
    ...
    low_confidence_messages = ChatbotMessage.objects.filter(
        tipo='bot',
        confianca__lt=0.7
    ).select_related('conversa').order_by('-timestamp')[:10]
...
def chatbot_create_ticket_from_conversation(request):
    ...
    try:
        ...
        if not conversation.usuario:
            return JsonResponse({
                'success': False,
                'error': 'Usuário não identificado na conversa'
            })
...
def chatbot_analytics_api(request):
    ...
    try:
        ...
        for i in range(days):
            ...
            count = ChatbotConversation.objects.filter(
                iniciada_em__date=date
            ).count()
        ...
        try:
            ...
            for name, value, tipo in configs:
                config, created = ChatbotConfiguration.objects.get_or_create(
                    nome=name,
                    defaults={'valor': value, 'tipo': tipo}
                )
                if not created:
                    config.valor = value
                    config.save()
