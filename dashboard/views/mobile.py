...
def mobile_tickets_ajax(request):
    ...
    if search:
        tickets = tickets.filter(
            Q(titulo__icontains=search) | Q(descricao__icontains=search) | Q(numero__icontains=search)
        )
    ...
    html = render_to_string(
        "mobile/_ticket_list_items.html",
        {"tickets": page_obj.object_list, "is_agent": bool(user_agent or request.user.is_staff), "is_mobile": True},
    )
    return JsonResponse(
        {
            "html": html,
...
from ..models import CategoriaTicket, Cliente, InteracaoTicket, PerfilAgente, Ticket
    ...
    if request.method == "POST" and "comment" in request.POST:
        ...
        if comment_text:
            InteracaoTicket.objects.create(
                ticket=ticket, usuario=request.user, conteudo=comment_text, tipo="comentario"
            )
        ...
        if titulo and descricao:
            ...
            try:
                ...
                customer = Cliente.objects.create(
                    nome=request.user.get_full_name() or request.user.username, email=request.user.email
                )
            ...
            if request.headers.get("X-Requested-With") == "XMLHttpRequest":
                return JsonResponse({"success": False, "error": "Título e descrição são obrigatórios."})
            messages.error(request, "Título e descrição são obrigatórios.")

    # Buscar categorias para o formulário
    categorias = CategoriaTicket.objects.all()

    context = {"categorias": categorias, "is_mobile": True}
...
def mobile_tickets_check_updates(request):
    ...
    try:
        ...
        if user_agent or request.user.is_staff:
            # Tickets do agente
            updated_tickets = Ticket.objects.filter(agente=request.user, atualizado_em__gt=last_check).exists()
        else:
            # Tickets do cliente
            customer = None
            try:
                customer = Cliente.objects.get(email=request.user.email)
                updated_tickets = Ticket.objects.filter(cliente=customer, atualizado_em__gt=last_check).exists()
            except Cliente.DoesNotExist:
                updated_tickets = False

        return JsonResponse({"hasUpdates": updated_tickets, "timestamp": timezone.now().timestamp()})
...
def mobile_chat_ticket(request, ticket_id):
    ...
    ticket = get_object_or_404(Ticket, id=ticket_id)
    return render(request, "mobile/chat_ticket.html", {"ticket": ticket, "is_mobile": True})
