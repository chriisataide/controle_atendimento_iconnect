...
from ..forms.whatsapp import WhatsAppAutoResponseForm, WhatsAppTemplateForm
...
from ..services.whatsapp_service import WhatsAppAnalyticsService, WhatsAppBusinessAPI, WhatsAppMessageProcessor
...
def whatsapp_dashboard(request):
    ...
    mensagens_hoje = WhatsAppMessage.objects.filter(timestamp__date=timezone.now().date()).count()
...
def whatsapp_conversations(request):
    ...
    conversations = WhatsAppConversation.objects.select_related("account", "contact", "agente", "ticket").order_by(
        "-iniciada_em"
    )
        ...
        if action == "assign_agent":
            ...
            if message_text:
                try:
                    ...
                    result = api.send_text_message(to=conversation.contact.whatsapp_id, text=message_text)
...
def whatsapp_contacts(request):
    ...
    if search:
        contacts = contacts.filter(
            Q(nome__icontains=search) | Q(profile_name__icontains=search) | Q(phone_number__icontains=search)
        )
...
def whatsapp_analytics(request):
    ...
    for account in accounts:
        ...
        analytics_data.append({"account": account, "metrics": metrics})
    ...
    for i in range(days):
        ...
        for account in accounts:
            analytics = WhatsAppAnalytics.objects.filter(account=account, data=date).first()
...
def whatsapp_api_send_message(request):
    ...
    if request.method == "POST":
        try:
            ...
            return JsonResponse({"status": "error", "message": "Erro ao enviar mensagem"}, status=400)
...
def whatsapp_api_conversation_messages(request, uuid):
    ...
    return JsonResponse({"messages": messages_data, "conversation_status": conversation.estado})
...
def whatsapp_api_stats(request):
    ...
    stats = {
        "mensagens_hoje": WhatsAppMessage.objects.filter(timestamp__date=today).count(),
        "conversas_ativas": WhatsAppConversation.objects.filter(estado="ativa").count(),
        "tickets_criados_hoje": WhatsAppConversation.objects.filter(ticket__criado_em__date=today).count(),
        "agentes_online": User.objects.filter(
            is_staff=True, last_login__gte=timezone.now() - timezone.timedelta(minutes=15)
        ).count(),
    }
