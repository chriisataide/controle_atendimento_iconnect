...
def estoque_dashboard(request):
    ...
    produtos_criticos = Produto.objects.filter(controla_estoque=True, estoque_atual__lte=F("estoque_minimo")).count()
    valor_total_estoque = (
        Produto.objects.filter(status="ativo", controla_estoque=True).aggregate(
    ...
    movimentacoes_hoje = MovimentacaoEstoque.objects.filter(data_movimentacao__date=hoje).count()

    # Produtos mais movimentados (últimos 30 dias)
    data_inicio = timezone.now() - timedelta(days=30)
    produtos_mais_movimentados = (
        MovimentacaoEstoque.objects.filter(data_movimentacao__gte=data_inicio)
        .values("produto__nome", "produto__codigo")
        .annotate(total_movimentacoes=Count("id"), quantidade_total=Sum("quantidade"))
    ...
    for i in range(7):
        ...
        entradas = MovimentacaoEstoque.objects.filter(data_movimentacao__date=data, tipo_operacao="entrada").count()
        saidas = MovimentacaoEstoque.objects.filter(data_movimentacao__date=data, tipo_operacao="saida").count()
        movimentacoes_por_dia.append({"data": data.strftime("%d/%m"), "entradas": entradas, "saidas": saidas})
    ...
    top_consumo_atendimentos = (
        ItemAtendimento.objects.filter(
            tipo_item="produto",
            adicionado_em__month=mes_atual,
            adicionado_em__year=ano_atual,
        )
        .values("produto__nome", "produto__codigo", "produto__estoque_atual", "produto__unidade_medida__sigla")
        .annotate(
    ...
    ultimas_mov_atendimento = (
        MovimentacaoEstoque.objects.filter(
            ticket_relacionado__isnull=False,
        )
        .select_related("produto", "ticket_relacionado", "usuario")
        .order_by("-data_movimentacao")[:10]
    )

    # Produtos com estoque crítico (lista)
    lista_criticos = (
        Produto.objects.filter(controla_estoque=True, estoque_atual__lte=F("estoque_minimo"), status="ativo")
    ...
    def get_queryset(self):
        ...
        if search:
            queryset = queryset.filter(
                Q(nome__icontains=search) | Q(codigo__icontains=search) | Q(codigo_barras__icontains=search)
            )
        ...
        if estoque_critico == "true":
            queryset = queryset.filter(controla_estoque=True, estoque_atual__lte=F("estoque_minimo"))
...
class ProdutoDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["movimentacoes"] = (
            MovimentacaoEstoque.objects.filter(produto=self.object)
            .select_related("usuario", "fornecedor", "tipo_movimentacao")
            .order_by("-data_movimentacao")[:10]
        )

        # Tickets relacionados
        context["tickets_relacionados"] = Ticket.objects.filter(movimentacaoestoque__produto=self.object).distinct()[:5]
...
class MovimentacaoListView(ListView):
    ...
    def get_queryset(self):
        queryset = MovimentacaoEstoque.objects.select_related("produto", "usuario", "fornecedor", "tipo_movimentacao")
    ...
    def get_context_data(self, **kwargs):
        ...
        context["movimentacoes_hoje"] = qs.filter(data_movimentacao__date=timezone.now().date()).count()
        return context
...
def relatorios_index(request):
    ...
    if tipo_relatorio == "estoque":
        ...
        queryset = Produto.objects.filter(status="ativo", controla_estoque=True).select_related(
    ...
    # Dados para relatório de movimentações
        ...
        resumo_tipos = queryset.values("tipo_operacao").annotate(
            total_movimentacoes=Count("id"), valor_total=Sum("valor_total")
        )
