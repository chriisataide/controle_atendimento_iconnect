...
def chat_dashboard(request):
    ...
    active_rooms_query = (
        ChatRoom.objects.filter(participants__user=user, participants__is_active=True, status="active")
        .annotate(
            last_message_time=Max("messages__created_at"),
            unread_count=Count(
                "messages",
                filter=Q(messages__created_at__gt=F("participants__last_read_message__created_at"))
    ...
    stats = {
        ...
        "today_messages": ChatMessage.objects.filter(
            room__participants__user=user, created_at__date=timezone.now().date()
        ).count(),
    }
...
def chat_room(request, room_id):
    ...
    participant = ChatParticipant.objects.filter(room=room, user=request.user, is_active=True).first()
    ...
    if not participant and request.user.is_staff:
        participant = ChatParticipant.objects.create(room=room, user=request.user, role="admin")

    # Mensagens recentes (últimas 50)
    messages_queryset = (
        room.messages.select_related("sender", "reply_to").filter(is_deleted=False).order_by("-created_at")[:50]
    )
    ...
    unread_messages = room.messages.exclude(sender=request.user).exclude(read_receipts__user=request.user)
...
def create_chat_room(request):
    ...
    if request.method == "POST":
        ...
        room = ChatRoom.objects.create(
            name=name, room_type=room_type, created_by=request.user, ticket_id=ticket_id if ticket_id else None
        )
        ...
        for user_id in participants_ids:
            try:
                ...
                ChatParticipant.objects.create(room=room, user=user, role="client" if not user.is_staff else "agent")
...
def api_send_message(request):
    ...
    try:
        ...
        message = ChatMessage.objects.create(
            room=room, sender=request.user, content=content, message_type=message_type, reply_to=reply_to
        )
...
def api_recent_rooms(request):
    ...
    rooms = (
        ChatRoom.objects.filter(participants__user=request.user, participants__is_active=True)
        .annotate(
            last_message_time=Max("messages__created_at"),
            unread_count=Count(
                "messages",
...
def api_create_ticket_from_chat(request, room_id):
    ...
    try:
        ...
        client_participant = room.participants.filter(role="client", is_active=True).select_related("user").first()
        ...
        ticket = Ticket.objects.create(
            titulo=title, descricao=description, cliente=cliente, prioridade=priority, created_by=request.user
        )
