...
def dashboard_financeiro(request):
    ...
    from ..models import ItemAtendimento, Ticket
    ...
    receita_chamados_mes = itens_mes.aggregate(total=Sum(valor_total_expr))["total"] or Decimal("0.00")
    ...
    receita_mes_anterior = itens_mes_anterior.aggregate(total=Sum(valor_total_expr))["total"] or Decimal("0.00")

    variacao_receita = (
        ((receita_chamados_mes - receita_mes_anterior) / receita_mes_anterior * 100)
    ...
    receita_por_categoria = (
        ItemAtendimento.objects.filter(
            ticket__status__in=["resolvido", "fechado"],
            ticket__fechado_em__date__gte=tres_meses_atras,
        )
        .values("ticket__categoria__nome")
    ...
    evolucao_mensal = (
        ...
        .annotate(mes=TruncMonth("ticket__fechado_em"))
        .values("mes")
        .annotate(total=Sum(valor_total_expr), qtd_tickets=Count("ticket", distinct=True))
        .order_by("mes")
    )
    evolucao_labels = (
        [e["mes"].strftime("%b/%y") for e in evolucao_mensal]
        if evolucao_mensal
        else ["Jan", "Fev", "Mar", "Abr", "Mai", "Jun"]
    )
    ...
    top_produtos = (
        ...
        .values("produto__nome", "produto__codigo", "tipo_item")
        .annotate(
    ...
    ultimos_chamados = (
        ...
        .filter(valor_total__isnull=False)
    ...
    top_clientes = (
        ...
        .values("ticket__cliente__nome", "ticket__cliente__empresa")
        .annotate(
            total_faturado=Sum(valor_total_expr),
            qtd_chamados=Count("ticket", distinct=True),
        )
...
def movimentacoes_lista(request):
    ...
    fluxo_mensal = (
        MovimentacaoFinanceira.objects.filter(data_movimentacao__gte=seis_meses_atras)
    ...
    cat_data = (
        MovimentacaoFinanceira.objects.values("categoria__nome").annotate(total=Sum("valor")).order_by("-total")[:6]
    )
...
def relatorios_financeiros(request):
    ...
    # Resumo executivo
    receita_total = MovimentacaoFinanceira.objects.filter(tipo="receita").aggregate(total=Sum("valor"))[
        "total"
    ] or Decimal("0.00")

    despesa_total = MovimentacaoFinanceira.objects.filter(tipo="despesa").aggregate(total=Sum("valor"))[
        "total"
    ] or Decimal("0.00")
    ...
    valor_atraso = Fatura.objects.filter(status="vencido").aggregate(total=Sum("valor"))["total"] or Decimal("0.00")
    pct_atraso = (valor_atraso / receita_total * 100) if receita_total else Decimal("0.00")

    # KPIs
    ticket_medio = Fatura.objects.filter(status="pago").aggregate(media=Avg("valor"))["media"] or Decimal("0.00")
    ...
    top_clientes = (
        Fatura.objects.filter(status="pago")
    ...
    top_despesas = (
        MovimentacaoFinanceira.objects.filter(tipo="despesa")
    ...
    dist_receitas = (
        MovimentacaoFinanceira.objects.filter(tipo="receita")
        .values("categoria__nome")
        .annotate(total=Sum("valor"))
        .order_by("-total")[:5]
    )
...
def api_gerar_relatorio(request):
    ...
    if request.method == "POST":
        try:
            ...
            movs = MovimentacaoFinanceira.objects.filter(data_movimentacao__range=[data_inicio, data_fim])

            total_receitas = movs.filter(tipo="receita").aggregate(t=Sum("valor"))["t"] or Decimal("0.00")
            total_despesas = movs.filter(tipo="despesa").aggregate(t=Sum("valor"))["t"] or Decimal("0.00")
            saldo = total_receitas - total_despesas
...
def centros_custo_lista(request):
    ...
    centros_custo = (
        CentroCusto.objects.select_related("responsavel", "gerente", "centro_pai").prefetch_related("subcentros").all()
    )
...
def centro_custo_detail(request, centro_id):
    ...
    movimentacoes_mes = centro_custo.movimentacoes.filter(
        data_movimentacao__month=mes_atual, data_movimentacao__year=ano_atual
    )

    total_despesas_mes = movimentacoes_mes.filter(tipo="despesa").aggregate(total=Sum("valor"))["total"] or 0

    total_receitas_mes = movimentacoes_mes.filter(tipo="receita").aggregate(total=Sum("valor"))["total"] or 0
...
def centros_custo_dashboard(request):
    ...
    centros_com_alerta = (
        CentroCusto.objects.filter(status="ativo", orcamento_mensal__gt=0)
        .annotate(
            percentual_usado=Sum(
            ...
            * 100
            / models.F("orcamento_mensal")
        )
        .filter(percentual_usado__gte=models.F("alerta_percentual"))
        .count()
    )
    ...
    departamentos = (
        CentroCusto.objects.filter(status="ativo")
        .values("departamento")
        .annotate(total_centros=Count("id"), total_orcamento=Sum("orcamento_mensal"))
        .order_by("departamento")
    )

    # Calcular orçamento total
    total_orcamento = CentroCusto.objects.filter(status="ativo").aggregate(total=Sum("orcamento_mensal"))["total"] or 0

    # Calcular gastos do mês atual
    total_gastos = (
        CentroCusto.objects.filter(status="ativo").aggregate(
            total=Sum(
                "movimentacoes__valor",
                filter=Q(
                    movimentacoes__tipo="despesa",
                ...
                stats_departamentos.append(
                    {
                        ...
                        "percentual_usado": (
                            (float(total_gasto) / float(total_orcamento) * 100) if total_orcamento > 0 else 0
                        ),
                    }
                )

            return JsonResponse({"departamentos": stats_departamentos, "status": "success"})
