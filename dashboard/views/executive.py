...
def executive_dashboard(request):
    ...
    # Alertas Críticos
    critical_alerts = AlertaKPI.objects.filter(is_resolved=False, severity__in=["high", "critical"])[:5]
    ...
    # Tickets resolvidos
    tickets_resolved = Ticket.objects.filter(status__in=["resolvido", "fechado"]).count()

    # Satisfação média
    satisfaction_avg = AvaliacaoSatisfacao.objects.aggregate(avg=Avg("nota_atendimento"))["avg"] or 0

    # SLA compliance
    tickets_with_sla = Ticket.objects.exclude(sla_deadline__isnull=True)
    if tickets_with_sla.exists():
        sla_compliant = tickets_with_sla.filter(resolvido_em__lte=F("sla_deadline")).count()
...
def executive_kpis_api(request):
    ...
    # 1. Volume de Tickets
    total_tickets_today = Ticket.objects.filter(criado_em__date=timezone.now().date()).count()
    ...
    tickets_with_sla = Ticket.objects.exclude(sla_deadline__isnull=True)
    sla_compliant = (
        tickets_with_sla.filter(resolvido_em__lte=F("sla_deadline")).count() if tickets_with_sla.exists() else 0
    )

    sla_compliance_rate = (sla_compliant / tickets_with_sla.count() * 100) if tickets_with_sla.count() > 0 else 0

    # 3. Satisfação do Cliente
    avg_satisfaction = AvaliacaoSatisfacao.objects.aggregate(avg_score=Avg("nota_atendimento"))["avg_score"] or 0

    # 4. Tempo Médio de Resolução (em horas)
    resolved_tickets = Ticket.objects.filter(status="fechado", resolvido_em__isnull=False)

    avg_resolution_time = 0
    if resolved_tickets.exists():
        avg_delta = resolved_tickets.aggregate(avg=Avg(F("resolvido_em") - F("criado_em")))["avg"]
        avg_resolution_time = round(avg_delta.total_seconds() / 3600, 1) if avg_delta else 0

    # 5. Produtividade dos Agentes
    agents_productivity = PerfilAgente.objects.filter(user__is_active=True).annotate(
        tickets_today=Count(
            "user__tickets_agente", filter=Q(user__tickets_agente__criado_em__date=timezone.now().date())
        )
    )

    avg_productivity = agents_productivity.aggregate(avg_tickets=Avg("tickets_today"))["avg_tickets"] or 0
...
def executive_charts_api(request):
    ...
    tickets_by_date = (
        Ticket.objects.filter(criado_em__gte=last_30_days)
        .annotate(date=TruncDate("criado_em"))
        .values("date")
    ...
    sla_by_category = CategoriaTicket.objects.annotate(
        ...
        sla_breached=Count("ticket", filter=Q(ticket__resolvido_em__gt=models.F("ticket__sla_deadline"))),
    ).values("nome", "total_tickets", "sla_breached")

    # Gráfico de Satisfação por Período
    satisfaction_trend = (
        AvaliacaoSatisfacao.objects.filter(avaliado_em__gte=last_30_days)
        .annotate(date=TruncDate("avaliado_em"))
        .values("date")
        .annotate(avg_score=Avg("nota_atendimento"))
        .order_by("date")
    )

    # Gráfico de Performance dos Agentes
    agents_performance = (
        PerfilAgente.objects.filter(user__is_active=True)
        .annotate(
        ...
        .values("user__first_name", "user__last_name", "tickets_resolved", "avg_satisfaction")
    )
    ...
    charts_data = {
        ...
        "agents_performance": {
            ...
            "satisfaction": [
                float(item["avg_satisfaction"]) if item["avg_satisfaction"] else 0 for item in agents_performance
            ],
        },
    }
...
def executive_alerts_api(request):
    ...
    tickets_at_risk = Ticket.objects.filter(
        status__in=["aberto", "em_andamento"], sla_deadline__lte=timezone.now() + timedelta(hours=2)
    ).count()
    ...
    recent_low_satisfaction = AvaliacaoSatisfacao.objects.filter(
        avaliado_em__gte=timezone.now() - timedelta(days=7), nota_atendimento__lt=3
    ).count()
    ...
    today_tickets = Ticket.objects.filter(criado_em__date=timezone.now().date()).count()

    avg_daily_tickets = (
        Ticket.objects.filter(criado_em__gte=timezone.now() - timedelta(days=30))
        .annotate(date=TruncDate("criado_em"))
        .values("date")
        .annotate(count=Count("id"))
...
def executive_widget_config(request):
    ...
    if request.method == "POST":
        ...
        return JsonResponse({"status": "success", "widget_id": widget.id})
