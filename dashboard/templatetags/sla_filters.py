...
def porcentagem_sla(ticket):
    ...
    try:
        ...
        tempo_total = calculator.calcular_tempo_util(
            ticket.data_criacao,
            ticket.prazo_sla,
            ticket.politica_sla
        )
        
        tempo_decorrido = calculator.calcular_tempo_util(
            ticket.data_criacao,
            timezone.now(),
            ticket.politica_sla
        )
...
def sla_dashboard_stats():
    ...
    stats = {
        ...
        'sla_critico': Ticket.objects.filter(
            status__in=['aberto', 'em_andamento'],
            prazo_sla__lt=agora
        ).count(),
        'sla_atencao': 0,
    ...
    tickets_ativos = Ticket.objects.filter(
        status__in=['aberto', 'em_andamento'],
        prazo_sla__gte=agora,
        politica_sla__isnull=False
    )
