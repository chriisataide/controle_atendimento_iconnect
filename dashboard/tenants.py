...
class TenantMiddleware(MiddlewareMixin):
    ...
    def process_request(self, request):
        ...
        if not tenant and hasattr(request, "user") and request.user.is_authenticated:
            membership = (
                TenantMembership.objects.filter(user=request.user, is_active=True).select_related("tenant").first()
            )
            if membership:
...
def tenant_context(request):
    ...
    if tenant and hasattr(request, "user") and request.user.is_authenticated:
        membership = TenantMembership.objects.filter(tenant=tenant, user=request.user, is_active=True).first()
        ctx["tenant_role"] = membership.role if membership else None
        ctx["user_tenants"] = Tenant.objects.filter(memberships__user=request.user, memberships__is_active=True)
    else:
...
def api_tenant_invite(request):
    ...
    membership = TenantMembership.objects.filter(tenant=tenant, user=request.user, role__in=["owner", "admin"]).first()
    if not membership:
...
def api_switch_tenant(request):
    ...
    try:
        ...
        membership = (
            TenantMembership.objects.filter(user=request.user, tenant__slug=slug, is_active=True)
