...
from .models_whatsapp import (
    ...
    WhatsAppMessage, WhatsAppTemplate, WhatsAppAnalytics
)
...
class WhatsAppBusinessAPI:
    ...
    def send_template_message(self, to: str, template_name: str, language: str = "pt_BR", components: List = None) -> Dict:
...
class WhatsAppMessageProcessor:
    ...
    def get_or_create_conversation(self, account: WhatsAppBusinessAccount, contact: WhatsAppContact) -> WhatsAppConversation:
...
Em breve nossa equipe entrará em contato. Obrigado!"""
    ...
    def check_auto_response(self, account: WhatsAppBusinessAccount, conversation: WhatsAppConversation, message: WhatsAppMessage):
        """Verifica e envia respostas automáticas"""
        try:
            from .models_whatsapp import WhatsAppAutoResponse
            
            # Busca respostas automáticas
            auto_responses = WhatsAppAutoResponse.objects.filter(
                account=account,
                ativo=True
            ).order_by('-prioridade')
    ...
    def should_trigger_auto_response(self, auto_response, conversation: WhatsAppConversation, message: WhatsAppMessage) -> bool:
        ...
        try:
            ...
            if auto_response.template:
                ...
                api_client.send_template_message(
                    to=conversation.contact.whatsapp_id,
                    template_name=auto_response.template.nome
                )
            elif auto_response.mensagem_texto:
                # Envia texto
                api_client.send_text_message(
                    to=conversation.contact.whatsapp_id,
                    text=auto_response.mensagem_texto
                )
...
class WhatsAppAnalyticsService:
    ...
    def update_daily_analytics(account: WhatsAppBusinessAccount, date=None):
        ...
        try:
            ...
            conversations = WhatsAppConversation.objects.filter(
                account=account,
                iniciada_em__gte=start_datetime,
                iniciada_em__lt=end_datetime
            )
    ...
    def get_account_metrics(account: WhatsAppBusinessAccount, days: int = 30) -> Dict:
        ...
        return {
            ...
            'tempo_resposta_medio': analytics.aggregate(
                avg_response=models.Avg('tempo_resposta_medio')
            )['avg_response'],
            'taxa_entrega': analytics.aggregate(
                total_sent=models.Sum('mensagens_enviadas'),
                total_delivered=models.Sum('mensagens_entregues')
            )
        }